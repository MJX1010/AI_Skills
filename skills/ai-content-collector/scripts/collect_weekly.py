...
def categorize_content(url, title):
    ...
    # 工具/资源
    if any(x in url_lower for x in ['github.com', 'huggingface.co', 'tools', 'app']):
        return "工具"
    
    # 研究论文
    if any(x in url_lower for x in ['arxiv', 'paperswithcode', 'research']):
        return "论文"
    
    # 技术文章
    if any(x in title_lower for x in ['教程', '指南', 'how', 'tutorial', 'guide']):
        return "教程"
...
def collect_weekly_content(week_info):
    """收集本周内容"""
    print(f"正在收集 第{week_info['issue_number']}期（{week_info['friday_display']}）的 AI 内容...")
...
def generate_weekly_markdown(contents, week_info):
    ...
    if contents['工具']:
        md += "## 🛠️ 工具\n\n"
        for i, item in enumerate(contents['工具'][:5], 1):
    ...
    if contents['文章']:
        md += "## 📖 文章\n\n"
        for i, item in enumerate(contents['文章'][:6], 1):
            md += f"### {i}. [{item['title']}]({item['url']})\n\n"
            md += f"{item['summary']}\n\n"
            md += f"> 来源：[{item['domain']}]({item['url']})\n\n"
        md += "---\n\n"
    ...
    if contents['论文']:
        md += "## 📚 论文\n\n"
        for i, item in enumerate(contents['论文'][:3], 1):
            md += f"### {i}. [{item['title']}]({item['url']})\n\n"
            md += f"{item['summary']}\n\n"
            md += f"> 来源：[{item['domain']}]({item['url']})\n\n"
        md += "---\n\n"
    ...
    if contents['教程']:
        md += "## 🎓 教程\n\n"
        for i, item in enumerate(contents['教程'][:3], 1):
            md += f"### {i}. [{item['title']}]({item['url']})\n\n"
            md += f"{item['summary']}\n\n"
            md += f"> 来源：[{item['domain']}]({item['url']})\n\n"
        md += "---\n\n"
    ...
    md += "\n---\n\n"
    md += "## 📮 订阅\n\n"
    md += "- 飞书知识库：[AI每周精选](https://xxx.feishu.cn/wiki/xxx)\n"
...
def save_weekly_content(contents, week_info):
    ...
    output_file = WEEKLY_DIR / f"weekly-{week_info['year']}-W{week_info['week']:02d}.md"
    output_file.write_text(md_content, encoding='utf-8')
    print(f"✅ 周刊已保存: {output_file}")
    
    # JSON 格式（便于后续处理）
    json_file = WEEKLY_DIR / f"weekly-{week_info['year']}-W{week_info['week']:02d}.json"
...
def main():
    parser = argparse.ArgumentParser(description='Collect weekly AI content (周刊模式)')
    ...
    print(f"=" * 60)
    print(f"AI每周精选 - 第{week_info['issue_number']}期")
