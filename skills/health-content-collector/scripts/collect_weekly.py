...
# 配置
SKILL_DIR = Path("/workspace/projects/workspace/skills/health-content-collector")
OUTPUT_DIR = Path("/workspace/projects/workspace/memory/health-content")
...
def categorize_content(url, title):
    ...
    # 运动健身
    if any(x in title_lower for x in ['运动', '健身', '锻炼', '跑步', '瑜伽', 'keep', '体能']):
        return "运动健身"
    
    # 饮食营养
    if any(x in title_lower for x in ['饮食', '营养', '食谱', '吃饭', '减肥', '瘦身', '膳食']):
        return "饮食营养"
    
    # 心理健康
    if any(x in title_lower for x in ['心理', '压力', '情绪', '焦虑', '抑郁', '冥想', '正念']):
        return "心理健康"
    
    # 睡眠健康
    if any(x in title_lower for x in ['睡眠', '失眠', '睡觉', '作息', '熬夜']):
        return "睡眠健康"
    
    # 医疗资讯
    if any(x in title_lower for x in ['疾病', '医疗', '医院', '医生', '症状', '治疗', '预防']):
        return "医疗资讯"
    
    # 生活妙招
    if any(x in title_lower for x in ['生活', '妙招', '窍门', '技巧', '方法', '实用']):
        return "生活妙招"
...
def collect_weekly_content(week_info):
    """收集本周内容"""
    print(f"正在收集 第{week_info['issue_number']}期（{week_info['friday_display']}）的健康生活内容...")
    
    all_results = []
    queries = CHINESE_QUERIES
...
def generate_weekly_markdown(contents, week_info):
    ...
    if contents['运动健身']:
        md += "## 🏃 运动健身\n\n"
        for i, item in enumerate(contents['运动健身'][:4], 1):
    ...
    if contents['饮食营养']:
        md += "## 🥗 饮食营养\n\n"
        for i, item in enumerate(contents['饮食营养'][:4], 1):
            md += f"### {i}. [{item['title']}]({item['url']})\n\n"
            md += f"{item['summary']}\n\n"
            md += f"> 来源：[{item['domain']}]({item['url']})\n\n"
        md += "---\n\n"
    ...
    if contents['心理健康']:
        md += "## 😊 心理健康\n\n"
        for i, item in enumerate(contents['心理健康'][:3], 1):
            md += f"### {i}. [{item['title']}]({item['url']})\n\n"
            md += f"{item['summary']}\n\n"
            md += f"> 来源：[{item['domain']}]({item['url']})\n\n"
        md += "---\n\n"
    ...
    if contents['睡眠健康']:
        md += "## 💤 睡眠健康\n\n"
        for i, item in enumerate(contents['睡眠健康'][:3], 1):
            md += f"### {i}. [{item['title']}]({item['url']})\n\n"
            md += f"{item['summary']}\n\n"
            md += f"> 来源：[{item['domain']}]({item['url']})\n\n"
        md += "---\n\n"
    ...
    if contents['医疗资讯']:
        md += "## 🏥 医疗资讯\n\n"
        for i, item in enumerate(contents['医疗资讯'][:3], 1):
            md += f"### {i}. [{item['title']}]({item['url']})\n\n"
            md += f"{item['summary']}\n\n"
            md += f"> 来源：[{item['domain']}]({item['url']})\n\n"
        md += "---\n\n"
    ...
    if contents['生活妙招']:
        md += "## ✨ 生活妙招\n\n"
        for i, item in enumerate(contents['生活妙招'][:4], 1):
            md += f"### {i}. [{item['title']}]({item['url']})\n\n"
            md += f"{item['summary']}\n\n"
            md += f"> 来源：[{item['domain']}]({item['url']})\n\n"
        md += "---\n\n"
    ...
    if contents['健康知识']:
        md += "## 📚 健康知识\n\n"
        for i, item in enumerate(contents['健康知识'][:3], 1):
            md += f"### {i}. [{item['title']}]({item['url']})\n\n"
            md += f"{item['summary']}\n\n"
            md += f"> 来源：[{item['domain']}]({item['url']})\n\n"
        md += "---\n\n"
    ...
    idx = 1
    for category in ['运动健身', '饮食营养', '心理健康', '睡眠健康', '医疗资讯', '生活妙招', '健康知识']:
    ...
    md += "\n---\n\n"
    md += "## 📮 订阅\n\n"
    md += "- 飞书知识库：[健康生活](https://xxx.feishu.cn/wiki/xxx)\n"
    md += "- GitHub：[MJX1010/AI_Skills](https://github.com/MJX1010/AI_Skills)\n\n"
    
    md += "---\n\n"
    md += "*本期周刊由 OpenClaw AI 自动生成，医疗相关内容建议专业审核*\n"
...
def save_weekly_content(contents, week_info):
    ...
    output_file = WEEKLY_DIR / f"health-weekly-{week_info['year']}-W{week_info['week']:02d}.md"
    output_file.write_text(md_content, encoding='utf-8')
    print(f"✅ 周刊已保存: {output_file}")
    
    # JSON 格式（便于后续处理）
    json_file = WEEKLY_DIR / f"health-weekly-{week_info['year']}-W{week_info['week']:02d}.json"
...
def main():
    parser = argparse.ArgumentParser(description='Collect weekly health content (周刊模式)')
    ...
    print(f"=" * 60)
    print(f"健康生活周刊 - 第{week_info['issue_number']}期")
