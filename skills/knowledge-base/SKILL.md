...
| 检查项 | 说明 | 阈值 |
|--------|------|------|
| **质量评分** | 来源可信度 + 标题质量 + 时效性 | ≥5.0分 |
| **数量限制** | 每个知识库最多收集 | 15条/天 |
| **来源白名单** | 可信来源加分（OpenAI、GitHub等） | +7~10分 |
| **黑名单过滤** | 低质量关键词过滤（震惊、必看等） | -1.5分 |
| **标题质量** | 长度20-60字、含年份/版本号加分 | +0.5~1分 |
| **相似度去重** | 标题相似度>70%视为重复 | 自动跳过 |

**质量评分算法：**
```
总分 = 来源可信度(0-10) + 标题质量(0-5) + 时效奖励(0-2)

来源可信度：
- 官方来源(openai.com, anthropic.com): 10分
- 可信技术媒体(github, huggingface): 8-9分
- 主流科技媒体: 6-7分
- 默认/未知来源: 3分
...
**使用v2版本：**
```bash
python skills/knowledge-base/scripts/daily_collect_v2.py
```
